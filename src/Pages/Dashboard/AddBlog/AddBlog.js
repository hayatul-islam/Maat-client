...
const AddBlog = () => {
  ...
  const submitHandler = async (e) => {
    ...
    try {
      await axios.post(`${apiLink}/blogs`, newBlog).then((result) => {
        if (result.status) {
          Swal.fire("Successfully!", "Added new Blog!", "success");
          e.target.reset();
        }
      });
    } catch (err) {}
    ...
    <div>
      ...
      <form onSubmit={submitHandler}>
        <div className="inputField addInput">
          <input ref={title} placeholder="Title" type="text" required />
          <input ref={sub_title} placeholder="Sub title" type="text" required />
          <input
            ...
            required
          />
          <input ref={category} placeholder="Category" required />
          <input ref={publish} value={new Date().toDateString()} required />
          <textarea
            ref={description}
            name="description"
            id=""
